...
function goSlide() {
  // 광클금지설정
  if(stsClick) return; // true일 때 함수를 빠져나감
  stsClick = true;
  setTimeout(() => {
    stsClick = false;
  }, TIME_ANI);
  ...
  // (2) 왼쪽버튼 클릭시 : 오른쪽으로 이동
    ...
    // 코드 실행 순서를 분리하여 비동기처리 => 변경사항이 시차를 두고 실행됨
    // 큐에서 대기하고 스택에서 실행된다
    setTimeout(() => {
  ...
  // 명령어 코드 - 콜스택 (call stack)에서 순서대로 실행된다
  // 타임아웃 메서드는 태스크 큐 (Task Queue)에 대기하다가
  // 콜스택의 명령어가 모두 실행된 후 큐에 쌓인 명령어를 스택으로 옮겨서 순서대로 실행
  // 큐: 대기공간, 스택: 실행공간
  //   console.log("1");
  //   setTimeout(() => {
  //     console.log("2");
